...
describe('executions', () => {
  ...
  describe('executionGet', () => {
    it('returns null for non-existent execution', async () => {
      ...
      const status = await executionGet(storage, taskHash, inHash);
  ...
  describe('executionGetOutput', () => {
    it('returns null for non-existent execution', async () => {
      const taskHash = 'a'.repeat(64);
      const inHash = 'b'.repeat(64);
      const output = await executionGetOutput(storage, taskHash, inHash);
  ...
  describe('executionList', () => {
    it('returns empty array for no executions', async () => {
      const list = await executionList(storage);
  ...
  describe('executionListForTask', () => {
    it('returns empty array for non-existent task', async () => {
      const taskHash = 'a'.repeat(64);
      const list = await executionListForTask(storage, taskHash);
      assert.deepStrictEqual(list, []);
    });
  ...
  describe('executionReadLog', () => {
    it('returns empty chunk for non-existent log', async () => {
      const taskHash = 'a'.repeat(64);
      const inHash = 'b'.repeat(64);
      const chunk = await executionReadLog(storage, taskHash, inHash, 'stdout');
    ...
    it('supports pagination', async () => {
      ...
      // Read first 5 bytes
      const chunk1 = await executionReadLog(storage, taskHash, inHash, 'stderr', {
        offset: 0,
        limit: 5,
      });
      ...
      // Read next 5 bytes
      const chunk2 = await executionReadLog(storage, taskHash, inHash, 'stderr', {
        offset: 5,
        limit: 5,
      });
  ...
  describe('taskExecute', () => {
    it('returns error for missing command IR', async () => {
      ...
      const result = await taskExecute(storage, taskHash, []);
    ...
    it('executes simple cp command successfully', async () => {
      ...
      // Execute
      const result = await taskExecute(storage, taskHash, [inputHash]);
    ...
    it('caches successful executions', async () => {
      ...
      // First execution
      const result1 = await taskExecute(storage, taskHash, [inputHash]);
      assert.strictEqual(result1.cached, false);
      assert.strictEqual(result1.state, 'success');

      // Second execution should be cached
      const result2 = await taskExecute(storage, taskHash, [inputHash]);
    ...
    it('force bypasses cache', async () => {
      ...
      // Force re-execution
      const result2 = await taskExecute(storage, taskHash, [inputHash], { force: true });
    ...
    it('captures stdout and stderr', async () => {
      ...
      const stdoutLog = await executionReadLog(storage, taskHash, inHash, 'stdout');
      const stderrLog = await executionReadLog(storage, taskHash, inHash, 'stderr');
    ...
    it('handles failed commands', async () => {
      ...
      const status = await executionGet(storage, taskHash, result.inputsHash);
      assert.ok(status);
