...
describe('packages', () => {
  ...
  describe('packageImport', () => {
    ...
    it('handles re-import of same package', async () => {
      ...
      const result1 = await packageImport(storage, zipPath);
      const result2 = await packageImport(storage, zipPath);
  ...
  describe('packageList', () => {
    ...
    it('lists multiple packages', async () => {
      ...
      await packageImport(storage, zip3);
  ...
  describe('packageResolve', () => {
    it('resolves package to hash', async () => {
      ...
      const resolvedHash = await packageResolve(storage, 'resolve-test', '1.0.0');
    ...
    it('throws for non-existent package', async () => {
      await assert.rejects(
        async () => await packageResolve(storage, 'nonexistent', '1.0.0'),
  ...
  describe('packageRemove', () => {
    it('removes package ref', async () => {
      ...
      let packages = await packageList(storage);
      assert.strictEqual(packages.length, 1);

      // Remove package
      await packageRemove(storage, 'remove-test', '1.0.0');

      // Verify package is gone
      packages = await packageList(storage);
      assert.strictEqual(packages.length, 0);
    });

    it('throws for non-existent package', async () => {
      await assert.rejects(
        async () => await packageRemove(storage, 'nonexistent', '1.0.0'),
        PackageNotFoundError
      );
    });
    ...
    it('removes only specified version', async () => {
      ...
      // Remove only v1
      await packageRemove(storage, 'multi-ver', '1.0.0');
  ...
  describe('packageExport', () => {
    it('exports empty package', async () => {
      ...
      const result = await packageExport(storage, 'export-test', '1.0.0', exportZip);
    ...
    it('exports package with input dataset', async () => {
      ...
      const result = await packageExport(storage, 'export-input', '1.0.0', exportZip);
    ...
    it('produces zip with same content as original', async () => {
      ...
      await packageImport(storage, originalZip);

      const exportedZip = join(tempDir, 'exported.zip');
      await packageExport(storage, 'roundtrip', '1.0.0', exportedZip);
    ...
    it('throws for non-existent package', async () => {
      ...
      await assert.rejects(
        async () => await packageExport(storage, 'nonexistent', '1.0.0', exportZip),
        PackageNotFoundError
      );
    });
  });
  ...
  describe('packages with tasks', () => {
    it('imports and reads package with single East task', async () => {
      ...
      const pkgObject = await packageRead(storage, 'single-task', '1.0.0');
    ...
    it('imports and reads package with two tasks (simpler than diamond)', async () => {
      ...
      const pkgObject = await packageRead(storage, 'two-task-test', '1.0.0');
    ...
    it('imports and reads package with diamond dependency (multiple tasks)', async () => {
      ...
      // Read the package object
      const pkgObject = await packageRead(storage, 'diamond-test', '1.0.0');
    ...
    it('roundtrip export of package with tasks preserves content', async () => {
      ...
      await packageExport(storage, 'task-roundtrip', '1.0.0', exportedZip);

      // Import into second repo
      const testRepo2 = createTestRepo();
      const storage2 = new LocalBackend(testRepo2);
      try {
        const result = await packageImport(storage2, exportedZip);
        assert.strictEqual(result.name, 'task-roundtrip');
        assert.strictEqual(result.version, '1.0.0');

        // Verify tasks are preserved
        const pkgObject = await packageRead(storage2, 'task-roundtrip', '1.0.0');
        assert.strictEqual(pkgObject.tasks.size, 1);
